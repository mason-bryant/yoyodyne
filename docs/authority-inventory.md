...
| conversation.authorize-reply | every role | `internal/chat/role.go` | `(*Session).authorize` | A parsed reply's proposals, concerns, research queries, evaluation, repository reads, ask, memory writes, or tracker actions that the role has no authority for. Nothing in the block is carried out. The repository block is the management roles' — the roles holding both `repository.read` and `repository.list` — so a developer's or a reviewer's conversation that names a path is refused here. The memory block is refused the same way to a role not holding `agent-context.mutate`, and what passes here is still written through `agent-context.authorize` below. The lane report block is refused the same way to a role not holding `lane-report.write`, which is the program manager alone, whether or not the block could be read, so nothing is written. |
...
| rolecapability.declared-ahead | the program manager | `internal/rolecapability/bundles.go` | `declaredAhead` | Nothing, by itself: it names each capability a bundle holds before any site asks for it — `readmodel.read`, `service.request-restart`, `report.file`, and `amendment.propose` — with what builds the site, so authority held and read by nothing is written down rather than inferred from a bundle. |
