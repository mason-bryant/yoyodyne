...
A role here is one of the six the harness has —
Lead Product Manager, architect, development manager, developer, reviewer, and
[program manager](designs/program-manager.md) — and a
...
document said once more, in capabilities rather than in role names: six bundles,
...
| conversation.authorize-reply | every role | `internal/chat/role.go` | `(*Session).authorize` | A parsed reply's proposals, concerns, research queries, evaluation, repository reads, ask, memory writes, restart request, or tracker actions that the role has no authority for. Nothing in the block is carried out. The repository block is the management roles' — the roles holding both `repository.read` and `repository.list` — so a developer's or a reviewer's conversation that names a path is refused here. The memory block is refused the same way to a role not holding `agent-context.mutate`, and what passes here is still written through `agent-context.authorize` below. The lane report block is refused the same way to a role not holding `lane-report.write`, which is the program manager alone, whether or not the block could be read, so nothing is written. The restart block is refused the same way to a role not holding `service.request-restart`, which is the program manager's alone; what passes is recorded as a request and nothing more. |
...
| conversation.contract.product-manager | the Lead Product Manager | `internal/chat/chat.go` | `productManagerContract` | Anything outside what the Lead Product Manager owns — the brief, the goals, and what is admitted to the backlog and in what order. |
...
| conversation.contract.program-manager | the program manager | `internal/chat/role.go` | `programManagerContract` | Writing code, closing or retiring anything, recording a triage decision or crossing a cap, issuing or settling a directive, and writing any document; what it wants outside its lane it says for the Lead Product Manager to decide. |
| conversation.lane-scope | the program manager | `internal/chat/lane.go` | `(*Session).refuseOutsideLane` | What the block alone shows is outside the lane, refused whole before anything runs: any lane-scoped tracker action — admitting, attributing, updating, labelling, reparenting, reprioritizing, parking, unparking, linking, unlinking — from an instance configured with no lane; a `label` removing the instance's own lane label; and a `create` carrying out a directive. |
| conversation.lane-at-act | the program manager | `internal/chat/lane.go` | `(*Session).laneRefusal` | A lane-scoped tracker action on an item that does not carry the lane label at the moment of the act, read from the tracker as the action runs, so a listing that has moved does not widen the lane; an item the tracker would not describe counts as outside it. A `link` making an outside item wait on a lane item is refused this way, and so is a `reparent` or a `create` naming a parent outside the lane. A `create` is never refused for the label: it carries the lane label in the write that admits it. |
| conversation.admission-gate | the Lead Product Manager, the program manager | `internal/chat/tracker.go` | `(*Session).admissionRefusal` | A `create` that would admit work where the project asks about every item, or where the goal it names is not one the operator approved. Decomposition underneath an admitted parent is deliberately not admission. A program manager's lane creation that this gate would refuse is put to the operator as a proposal naming the lane instead, and is admitted only on their approval. |
...
| conversation.repair-is-stale | the Lead Product Manager | `internal/backlogrepair/backlogrepair.go` | `Judge` | A `repair` of state the records do not say is stale: a status with unfinished work behind it, a dependency the tracker does not hold as closed, an attribution that still resolves. The correction rests on the records read at the moment of the act rather than on the listing the request was written against. |
| conversation.repair-hold | the Lead Product Manager | `internal/backlogrepair/backlogrepair.go` | `heldFor` | Any `repair` of an item somebody still has to release — a stoppage nobody has decided about, a change that exists only on a preserved branch, an active directive that pauses the work it affects. Such state is reported with the reason restated and never corrected, and holds that could not be read hold every item. |
...
| sidestream.no-action | every role | `internal/sidestream/sidestream.go` | `Permits` | Everything a side conversation might ask for beyond reading the tracker and the evidence its role was given: a work-item mutation, an admission, a reorder, a decomposition, a triage decision, a proposal, a concern, research, an evaluation, an ask, and a memory write. A side thread judges and drafts, and every intent it forms is ratified by the main thread through its own single-threaded path. |
| conversation.side-stream-narrows | every role | `internal/chat/sidestream.go` | `(Authority).OnSideStream` | A role's own conversation authority, on a side thread, reduced to what a side thread holds. Every answer is the role's answer *and* the side thread's, so the narrowing can only take authority away — and the per-agent knob that chooses side threads at all selects behavior and never authority. |
| sidestream.side-thread-contract | every role | `internal/sidestream/contract.go` | `SideThreadContract` | Any action from a side thread: it judges, answers, reads, and drafts, and every intent it forms is ratified by the main thread. What it promises is best effort until that ratification, and the surface carrying its answer says so. |
| sidestream.reply-carries-no-authority | every role | `internal/sidestream/contract.go` | `ReadReply` | A side thread's reply carrying any harness block but its own draft, whole, whether or not the role read the contract. The turn is spent and the prose survives; nothing the block asked for happens. |
| sidestream.side-prompt | every role | `internal/chat/sidestream.go` | `SidePrompt` | The role's own conversation contract standing in on a side thread: what is sent is who the role is and the side thread's boundary, not the blocks and authority the main thread has. |
...
| exchange.ask-authority | every role | `internal/chat/exchange.go` | `refuseUnauthorizedAsk` | An ask from a role not holding `exchange.ask`, an ask a role puts to its own role, and an ask put to a role not holding `exchange.answer`. |
...
| artifact.owner | the Lead Product Manager, the architect | `internal/artifact/ownership.go` | `Owner` | A kind of document nobody owns: an unknown kind has no owner rather than a default one. |
...
| agent-context.authorize | every role | `internal/agentcontext/agentcontext.go` | `Authorize` | A memory write by a role whose bundle does not hold `agent-context.mutate`: the Lead Product Manager, the architect, the development manager, and the program manager keep a memory of their own, and the developer and the reviewer keep none, because accumulation and independence cannot live in one identity. |
| agent-context.unauthorized-error | every role | `internal/agentcontext/agentcontext.go` | `ErrUnauthorized` | What every refused memory write returns, so a caller can tell refused authority from a malformed revision or a spent budget. |
| agent-context.merge-through-the-one-door | every role | `internal/agentcontext/merge.go` | `(Conclusion).Conclude` | A side conversation's merge back that skipped the memory machinery: the merge composes a revision and hands it to `Remember`, so it is refused, redacted, budgeted, and audited exactly as any other write, and a role that keeps no memory merges nothing whatever its side thread found out. Concluding is one call that merges and then records the stream as ended, in that order — which is the pairing offered rather than a boundary enforced, because `runstate.SideStreamStore.Save` stays reachable and the store cannot reach this package to refuse it. It is not `sidestream.no-action` weakened: a side thread still may not ask for a memory write, and this is the harness writing when the thread has already ended. |
...
| amendment.operator-decides | every role | `internal/cli/amendment.go` | `decideAmendment` | An operator naming the authority a change is decided under: it is the artifact's owner and is never asked for, so a design change cannot be decided under the Lead Product Manager's authority. |
...
| protectedpath.grant-problems | the Lead Product Manager, the development manager | `internal/protectedpath/provider.go` | `GrantProblems` | A work item granting a path the provider refuses, asked at every door into the queue so an item is not admitted for work no run can do. |
| protectedpath.condition-problems | the Lead Product Manager, the development manager | `internal/protectedpath/condition.go` | `(Homes).ConditionProblems` | A work item whose done-conditions name a path under an artifact home, or a document one owns, without a grant for it, asked at every door into the queue and again by the run, so an item is not admitted for a condition no developer run may satisfy. |
...
| run.refuse-ungranted-condition | the developer | `internal/orchestrator/pipeline.go` | `(Pipeline).refuseUngrantedCondition` | Starting on an item whose done-conditions name a document under an artifact home the item does not grant, before the item is claimed: a condition no diff can satisfy is refused at the start of the run rather than parked at the end of it. |
...
| backend.read-only-role | the Lead Product Manager, the architect, the development manager, the reviewer, the program manager | `internal/backend/claudecode/backend.go` | `readOnlyRole` | Names the roles that reason over supplied evidence, which is what the tool refusal below is decided from. |
...
| backend.no-tools-for-read-only | the Lead Product Manager, the architect, the development manager, the reviewer, the program manager | `internal/backend/claudecode/backend.go` | `(Backend).Run` | Tools granted to a role that reasons over bounded supplied evidence, and an unsupported role, before the provider is reached. |
...
| run.land-under-lease | the harness | `internal/orchestrator/pipeline.go` | `(*activeRun).leaseLanding` | Cutting a landing checkout or running a landing check without first holding that branch's landing lease, and waiting for it past what the landing ahead may take. |
...
| rolecapability.role-bundles | every role | `internal/rolecapability/bundles.go` | `bundles` | Nothing, by itself: it is the statement of what each of the six roles holds, derived from this inventory and written in Go so that configuration cannot widen it. It is listed because it is the other half of this document, and a change to it is a change to what the harness says a role may do. |
| rolecapability.declared-ahead | the program manager | `internal/rolecapability/bundles.go` | `declaredAhead` | Nothing, by itself: it names each capability a bundle holds before any site asks for it — `readmodel.read`, `report.file`, and `amendment.propose` — with what builds the site, so authority held and read by nothing is written down rather than inferred from a bundle. |
...
| `internal/cli/chat.go` | `prepareChat` | Builds the conversation's components and refuses a backend this build cannot launch. It is the half of opening a conversation that does not hold it, shared by the main thread and by a side thread held beside it; which of the two a message reaches is behaviour the agent's configuration selects, and what either may do is decided in the chat package's table and in `sidestream.no-action`. |
| `internal/cli/directive.go` | `withdrawDirective` | Takes a directive out of force, reading `--as` for the role the withdrawal is made under. The role is attribution — it is what a Slack thread the directive came from is answered in the voice of — and decides nothing about who may withdraw; the record refuses only a role the harness does not have. |
...
| `internal/directive/directive.go` | `(Directive).Withdraw` | Writes a withdrawal onto the record, the role it was made under included. The role is recorded exactly as `ReceivedBy` is — attribution, never routing or permission — and the only refusal made on it is of a role the harness has no voice for. |
...
| `internal/notify/conversation.go` | `fromUnresolvedTrackerRefusal` | Turns a recorded refusal the harness woke a role to correct and got another refused block from into something the operator is told. It reads the role the record names so the message can say whose actions are still lost; both refusals already happened, in the conversation. |
...
| `internal/orchestrator/orchestratortest/forge.go` | `(*Forge).SetTargetProtection` | Tells the same test double which branch protection to report, for a fixture that builds the forge before its test says. Nothing outside tests imports the package, and it refuses nothing. |
...
| `internal/runstate/supervision.go` | `(*SupervisionStore).Lease` | Serializes the product's supervisor against a second copy of itself, so two of them do not each restart what the other stopped. It decides which process may supervise, never which role may do anything. |
