...
| agent-context.merge-through-the-one-door | every role | `internal/agentcontext/merge.go` | `(Conclusion).Conclude` | A side conversation ending without its substance being written, and a merge that skipped the memory machinery. Concluding is one call that merges and then records the stream as ended, and the merge composes a revision and hands it to `Remember`, so it is refused, redacted, budgeted, and audited exactly as any other write — and a role that keeps no memory merges nothing, whatever its side thread found out. It is not `sidestream.no-action` weakened: a side thread still may not ask for a memory write, and this is the harness writing when the thread has already ended. |
