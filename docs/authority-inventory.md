...
| conversation.authorize-reply | every role | `internal/chat/role.go` | `(*Session).authorize` | A parsed reply's proposals, concerns, research queries, evaluation, repository reads, ask, or tracker actions that the role has no authority for. Nothing in the block is carried out. The repository block is the management roles' — the roles holding both `repository.read` and `repository.list` — so a developer's or a reviewer's conversation that names a path is refused here. |
...
| conversation.repair-hold | the product manager | `internal/backlogrepair/backlogrepair.go` | `heldFor` | Any `repair` of an item somebody still has to release — a stoppage nobody has decided about, a change that exists only on a preserved branch, a directive in force that pauses the work it affects. Such state is reported with the reason restated and never corrected, and holds that could not be read hold every item. |
