...
| rolecapability.declared-ahead | the program manager | `internal/rolecapability/bundles.go` | `declaredAhead` | Nothing, by itself: it names each capability a bundle holds before any site asks for it — `readmodel.read`, `lane-report.write`, `report.file`, and `amendment.propose` — with what builds the site, so authority held and read by nothing is written down rather than inferred from a bundle. |
