...
| conversation.lane-scope | the program manager | `internal/chat/role.go` | `refuseOutsideLane` | A tracker action the role holds only inside its own lane — admitting, attributing, updating, labelling, reparenting, reprioritizing, parking, unparking, linking, unlinking — asked for on an item outside it. No lane is enforced yet, so no item is inside one and every such action is refused whole; the lane itself is the next child of the program manager's work. |
...
| `internal/cli/passevents.go` | `(instanceConversations).InFlight` | Asks whether a turn is in flight on a program manager instance's conversation, naming the role because a conversation is identified by its agent and its role. It decides whether a pass is skipped this pull, never what the instance may do. |
...
| `internal/orchestrator/programmanagerpass.go` | `(Trigger).pass` | Decides whether a program manager instance's pass is taken at this pull — its schedule, the events past its cursor, a turn already in flight — and wakes the instance for it. Which instance is woken is what its configured triggers name, and the turn is authorized exactly as a conversation an operator opens with it is, under `conversation.authority-table`. |
