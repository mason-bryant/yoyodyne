...
| agent-context.authorize | every role | `internal/agentcontext/agentcontext.go` | `Authorize` | A memory write by a role whose bundle does not hold `agent-context.mutate`: the product manager, the architect, and the development manager keep a memory of their own, and the developer and the reviewer keep none, because accumulation and independence cannot live in one identity. |
