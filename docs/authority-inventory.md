...
| protectedpath.set | the developer | `internal/protectedpath/protectedpath.go` | `Protect` | Builds the default-deny set: the harness's own project directory and the artifact homes the roles upstream of a developer own. |
| protectedpath.refused | the developer | `internal/protectedpath/protectedpath.go` | `(Set).Refused` | Every path a change touched that sits inside a protected home and the work item did not grant. |
...
| separation.topology | the developer, the reviewer | `internal/separation/separation.go` | `CheckTopology` | A sequence that can reach a step moving the target branch without having crossed, on every path to it, both a step that runs the project's configured checks and a step that returns a verdict. Every path rather than some path: one route through the review and one around it is a sequence that can promote unjudged work. |
