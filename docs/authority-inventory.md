...
product manager, architect, development manager, developer, reviewer, and
...
| conversation.contract.program-manager | the program manager | `internal/chat/role.go` | `programManagerContract` | Writing code, closing or retiring anything, recording a triage decision or crossing a cap, issuing or settling a directive, and writing any document; what it wants outside its lane it says for the product manager to decide. |
...
| conversation.admission-gate | the product manager, the program manager | `internal/chat/tracker.go` | `(*Session).admissionRefusal` | A `create` that would admit work where the project asks about every item, or where the goal it names is not one the operator approved. Decomposition underneath an admitted parent is deliberately not admission. A program manager's lane creation that this gate would refuse is put to the operator as a proposal naming the lane instead, and is admitted only on their approval. |
...
| protectedpath.refused | the developer | `internal/protectedpath/protectedpath.go` | `(Set).Refused` | Every path a change touched that sits inside a protected home, or is a held export, and the work item did not grant — and every role definition, whatever the item grants. |
...
| protectedpath.role-definitions | the developer | `internal/protectedpath/roles.go` | `RoleDefinitions` | A run writing a role definition, which would let it widen a role's authority: no grant reaches the directory, at admission, before a run starts, or in the diff gate. |
| protectedpath.grant-problems | the product manager, the development manager | `internal/protectedpath/provider.go` | `GrantProblems` | A work item granting a path the provider refuses, or the role definitions, asked at every door into the queue so an item is not admitted for work no run can do or given authority over a role. |
| protectedpath.condition-problems | the product manager, the development manager | `internal/protectedpath/condition.go` | `(Homes).ConditionProblems` | A work item whose done-conditions name a path under an artifact home, or a document one owns, without a grant for it, asked at every door into the queue and again by the run, so an item is not admitted for a condition no developer run may satisfy. |
