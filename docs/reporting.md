...
recorded, counts only where every agent asks for the same thing; on a project
whose agents differ it cannot be attributed, and a hold invented over it would
send somebody to look at roles that are being served. Failover working is the
