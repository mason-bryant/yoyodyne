...
This tier is the one part of the channel that can be declined, and the Slack app's
`im:write` scope is how. An app without it reports into the channel on exactly the
same schedule with exactly the same messages, and this tier alone goes quiet,
saying so once in the sink's own log. There is deliberately no configuration
setting that turns it off — a switch in a file is a switch somebody sets and
forgets, and what would be forgotten is the message that says the line has
stopped.
