...
| A claim in the notes guard's own suite, [`scripts/bd-notes-guard-test.sh`](../scripts/bd-notes-guard-test.sh), that no longer holds | `internal/cli` (`notesguard_repository_test.go`) | Read the claim it named and fix [`scripts/bd-notes-guard.sh`](../scripts/bd-notes-guard.sh) or its Python half. The guard is a hook, so its value is entirely in writes it refuses, and it first fires on the day somebody is about to destroy a goal attribution. |
| `scripts/bd-notes-guard.py` reading an attribution differently from `goal.NamedIn` — a different prefix, different trimming, a different one of several winning | `internal/cli` (`notesguard_repository_test.go`) | Re-match the Python to the Go. One rule is implemented twice in two languages and a diff to either side shows none of the disagreement; worse, every way it drifts fails open the same way — the guard reads no attribution, treats the write as one against an unattributed item, and goes on passing its own suite while protecting nothing. |
| The `PreToolUse` block in `CLAUDE.md` and `AGENTS.md` failing to parse, naming a script that is not there, reading differently in the two files, or disagreeing with a `.claude/settings.json` that carries the paste | `internal/cli` (`notesguard_repository_test.go`) | Fix the block, mirroring the fix into both, and into the settings file if it has been pasted. An operator pastes it by hand — Claude Code refuses an agent's write to a settings file, so no run can land it — and a block naming a moved script installs a hook that fails open, which Claude Code reports as a line in a transcript nobody reads. A settings file with no `PreToolUse` entry at all skips rather than fails, because nothing running here can put one there — and that skip expires the moment somebody pastes the block, so whoever does turns it into a failure in the same commit. |
| The command in that block failing to refuse when it is actually run — a `$CLAUDE_PROJECT_DIR` that resolves to nothing, quoting that breaks on a path with a space in it | `internal/cli` (`notesguard_repository_test.go`) | Fix the command in the block. This is the only check that executes what an operator pastes rather than reading it: a block can parse, name the right script, and still resolve to nothing, and Claude Code reports that as a non-blocking error and lets the call through, so the hook is unguarded and looks guarded. Nothing here can wire the hook, so running its command is as close as a check gets to the interactive refusal. |
| `developerSettings` not putting a developer run's Bash through the notes guard, naming a script that is not there, or reaching it through `$CLAUDE_PROJECT_DIR` instead of the worktree the run was given | `internal/backend/claudecode` (`backend_test.go`) | Restore the `PreToolUse` hook, or the script it names. This is the harness half of the same guard the instruction files tell an operator to paste for interactive sessions, and today it is the only half that is wired; one script serves both wirings, so once the paste lands the refusal is identical rather than merely similar. A hook command that does not resolve is a non-blocking error that lets the call through, so every way of getting the path wrong fails open and looks like a run that was guarded. |
