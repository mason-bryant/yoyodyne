...
person saying so. Nothing else skips, because the whole
point is that a machine that simply never installed Node stops reading as a
green run — which is what `make test` printing the render test's own line, after
the suite, is there to make visible either way:
