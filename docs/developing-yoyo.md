...
| The `PreToolUse` block in `CLAUDE.md` and `AGENTS.md` failing to parse, naming a script that is not there, or reading differently in the two files | `internal/cli` (`notesguard_repository_test.go`) | Fix the block, mirroring the fix into both. An operator pastes it into `.claude/settings.json` by hand — Claude Code refuses an agent's write to a settings file — so a block naming a moved script installs a hook that fails open, and Claude Code reports that as a line in a transcript nobody reads. |
| The prefix `scripts/bd-notes-guard.py` looks for drifting from `goal.AttributionPrefix` | `internal/cli` (`notesguard_repository_test.go`) | Re-copy the constant. The guard is Python and the record is Go, so the string is duplicated with nothing but this holding the two together — and the drift is silent in the worst direction: the guard stops recognising any attribution, treats every replacing write as touching an unattributed item, and goes on passing its own suite while protecting nothing. |
