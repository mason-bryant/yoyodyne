...
| `developerSandboxSettings` not putting a developer run's Bash through the notes guard, or naming a script that is not there | `internal/backend/claudecode` (`backend_test.go`) | Restore the `PreToolUse` hook, or the script it names. This is the harness half of the same guard the instruction files tell an operator to paste for interactive sessions; one script serves both so the refusal is identical, and a hook whose command is missing is a non-blocking error that lets the call through. |
