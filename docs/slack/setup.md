...
| `a direct conversation with <member> could not be opened` | Usually `conversations.open: missing_scope` on an app installed before the manifest asked for `im:write`, or a member id that is not in this workspace. The messages this affects are the two that report the harness itself degraded — a stale session build, and the harness having started nothing at all — and the advisory-once notice that one value the template has improved is available. All of them stand in the channel either way; reinstall from *OAuth & Permissions* and the next one reaches them. Note that the improvement is not re-sent once the channel has it — it speaks exactly once per improvement, so the one whose direct message failed is read back from the channel or from `yoyo config drift`, not resent. |
