...
| `slack refused conversations.open: missing_scope` | The app was installed before the manifest asked for `im:write`. Reinstall it from *OAuth & Permissions*. Nothing is lost while it stands: what was already posted in the channel stands and keeps its place, and the message that was trying to reach you is sent as soon as the app can open a conversation. What it costs meanwhile is pace — the sink waits this refusal out for minutes rather than seconds, the way it does every refusal only a person can clear, so the channel runs late until you reinstall. |
