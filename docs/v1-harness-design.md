...
- **A refusal is reported as a refusal.** A protected branch declining the merge because a required check has not run or a review is missing is the repository's rules being applied, not the harness failing, so the run reports which requirement was unmet — the forge's merge state and its own message — rather than a generic failure.
