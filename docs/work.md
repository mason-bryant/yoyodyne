...
`yoyo pause` is the switch for stopping what a quiet session spends. Nor does it
stop the claim audit, which is read before it for the same reason: the brake
that holds intake is placed exactly when runs are failing one after another,
which is when a claim is most likely to have just died.
