...
- `workflow_divergence` is why the run stopped being observed: the definition
  sent it somewhere it did not go, refused an outcome it produced, or could not
  be stepped at all. A run carrying one is a run to read before the definition is
  trusted with anything, and it is the whole measure of the trial — a clean soak
  is N real items observed with none of these.
