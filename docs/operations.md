...
1. **Where the harness stands** — a tile for each of the four lines: running
   ...
   model's own vocabulary for whose move each entry is: its figure is what
   waits on the operator, and beside it, out of the line's whole count — the
   figure the terminal prints — what waits on the architect, the development
   manager, the product manager, and the harness, each named only where it is
   not zero. The tile asks for attention when something waits on the operator,
   not when sixty things wait on a role. Two more tiles carry what landed today
   and in the last seven days, and what it cost, the latter prefixed `≥` or `at
   least` where a record that should be in it could not be read.
