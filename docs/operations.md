...
3. **Where the work stands** — the pipeline, read left to right: admitted items;
   ...
   Under it, in words, how many things wait on a person. Every stage and every
   pile is a button that opens [the list of the items in it](#opening-a-work-item).
...
**A grouping's items.** In *Where the work stands*, each stage — Admitted, Held
back, Startable, Running, Landed — and each pile under one (`held for a
person`, `pullable, and nothing is choosing`, `developing`, and the rest; the
week's landed figure is a grouping of its own beside today's) opens a list of
the work items in it, by title, with the id under each and, where the pipeline
...
and dropping the hidden ones, a pop-up among them unless it is open — one file
per scenario — `quiet`, `busy`, `held`, `degraded`, `unreadable`, `loading`,
`throughput-pending`, `throughput-refused`, `throughput-stale`, `refused`,
`unreachable`, `wrong-token`, `stale`, and `signin` for the page, and `card`,
`card-loading`, `card-missing`, `card-refused`, `grouping`, `grouping-pile`,
`grouping-landed`, `grouping-empty`, `grouping-error`, `grouping-loading`,
`grouping-card`, and `closed` for the pop-ups, each opened by clicking what a
reader would click — which together show every section and each pop-up in each
of its four states. They are golden files:
