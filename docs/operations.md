...
passes on the request. For six days in September 2026 eight queued merges
...
Where the fix goes is the target branch: the held requests are cut from it and
inherit what it carries, as the eight inherited the red test, so no repair of
one run's own change reaches the others. The fix lands on the target branch,
is published as any change is, and the forge performs every held merge once
