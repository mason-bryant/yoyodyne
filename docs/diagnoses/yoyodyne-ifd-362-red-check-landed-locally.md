...
harness commit the worktree stood at where the run had made one — and
`integrate` reads it before it takes the promotion lease or writes a phase. It
refuses, with `ErrIntegrationUnearned`, when the record carries a standing
check failure or protected-path refusal, no passing checks at all, passing
checks for a different attempt or a different commit, no approving verdict, or
passing checks over content other than what the worktree holds at the moment
of promotion. Recording a failure clears the evidence; a new attempt
invalidates it by moving the attempt count; an edit to the tree invalidates it
by moving the content.

The content is what binds the evidence to a revision on a project that has made
no commit to bind it to. A publishing run commits each attempt before its checks
run, so its evidence names that commit; a project that does not publish commits
nothing until the promotion itself, and until this change the only binding on
its ordinary path was the attempt count, which says when the checks ran and
nothing about what they ran over. `gitworktree.Manager.ContentIdentity` names
the change as the worktree holds it — a digest over the base, every path the
change touches, and the blob each one's content is, read without writing
