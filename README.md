...
- [Provider plugins](docs/provider-plugins.md) — running yoyo on a coding CLI or
  harness it ships no adapter for: the six answers a provider has to give, the
  rule format for describing one in configuration, and why a plugin never decides
  how long to wait.
