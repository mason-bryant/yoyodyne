...
Three things guard a loop that no longer ends. An item whose run fails *before it
starts* is left alone until something about the item changes, so a start the
harness cannot get past is not retried every minute forever. Runs blocking one
after another with nothing landing between them hold intake at
...
silence. `--budget <usd>` caps what one session spends.
