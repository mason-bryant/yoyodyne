...
Whichever way you got it, this and every document it links to call the binary
`yoyo` and assume it is on your `PATH`. Run it from your own project: it
discovers its configuration by searching upwards from the current directory.
...
behind. That is the whole loop, and
[The conversation](docs/conversation.md) is the document that covers the rest of
it.
...
[How work flows once you approve it](docs/work.md) has the full behavior, and
the [configuration guide](docs/configuration.md#publishing-through-pull-requests)
...
- [The conversation](docs/conversation.md) — proposals and batches, steering the
  work, directives, talking to the other agents, and what the conversation looks
  like on a terminal.
...
- [Operations and recovery](docs/operations.md) — pausing and resuming, provider
  limits and stalls, recovering interrupted runs, what became of the runs, and
  following one as it happens.
...
- [Setting up Slack reporting](docs/slack/setup.md) — an empty workspace to live
