...
// TestReviewEvidenceKeepsTheSharedConstraintsAheadOfWhatVaries pins
// yoyodyne-ifd.84 at the review gate. The invariants lead the evidence because
// they are the harness's own and the rest is the developer's; what this adds is
// that the constraints binding every change lead the ones selected for this one,
// so two reviews of two different changes share a prefix a provider can cache.
// The guard beside it is that nothing about the change went missing to get
// there.
func TestReviewEvidenceKeepsTheSharedConstraintsAheadOfWhatVaries(t *testing.T) {
	t.Parallel()

	// What the harness renders for a repository whose invariants are one
	// repository-wide constraint plus one the change happened to touch.
	wide := "# Architectural invariants\n\n## harness-owns-git: Only the harness commits\n\n" +
		"Scope: the whole repository\nEstablished by: yoyodyne-ifd.1\n\n" +
		"Must hold:\n\nNo agent commits, pushes, or merges.\n\nWhy:\n\nIntegration is serial.\n\n"
	scoped := "## one-writer-per-item: One writer per item\n\n" +
		"Scope: internal/runstate\nEstablished by: yoyodyne-ifd.2\n\n" +
		"Must hold:\n\nEvery path into an in-flight run reserves it.\n\nWhy:\n\nTwo writers is a race.\n\n"
	completeness := "1 of the 2 active invariant(s) recorded here were selected as relevant.\n"

	first := newRequest(nil)
	first.Invariants = wide + completeness
	first.Changes = gitworktree.ChangeDiff{Status: " M console.go", Patch: "diff --git a/console.go b/console.go\n+renderPlain()\n"}
	first.Checks = []checks.Result{{Command: "go test ./internal/console/", Passed: true}}

	second := newRequest(nil)
	second.WorkItemID = "yoyodyne-reserve"
	second.Context = "# Assigned work item\n\nID: yoyodyne-reserve\nTitle: Reserve before work\n"
	second.Invariants = wide + scoped + "2 of the 2 active invariant(s) recorded here were selected as relevant.\n"
	second.Changes = gitworktree.ChangeDiff{Status: " M store.go", Patch: "diff --git a/store.go b/store.go\n+reserve()\n"}
	second.Checks = []checks.Result{{Command: "go test ./internal/runstate/", Passed: true}}

	prompts := []string{reviewEvidencePrompt(first), reviewEvidencePrompt(second)}
	shared := commonPrefix(prompts[0], prompts[1])
	if !strings.Contains(shared, "## harness-owns-git: Only the harness commits") || !strings.Contains(shared, "Integration is serial.") {
		t.Errorf("the repository-wide invariant is not on the shared prefix:\n%s", shared)
	}
	for _, unwanted := range []string{"one-writer-per-item", "were selected as relevant", "# Untrusted review evidence"} {
		if strings.Contains(shared, unwanted) {
			t.Errorf("the shared prefix carries %q, which varies between reviews:\n%s", unwanted, shared)
		}
	}

	// The guard: each review still carries the change it is judging.
	for index, want := range [][]string{
		{"yoyodyne-task", "console.go", "renderPlain()", "go test ./internal/console/"},
		{"yoyodyne-reserve", "store.go", "reserve()", "go test ./internal/runstate/", "one-writer-per-item"},
	} {
		for _, evidence := range want {
			if !strings.Contains(prompts[index], evidence) {
				t.Errorf("review evidence %d lost %q:\n%s", index, evidence, prompts[index])
			}
		}
	}
}
