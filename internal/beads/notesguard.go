...
func DestroyedAttribution(command string) string {
	for _, words := range simpleCommands(command) {
		...
		return fmt.Sprintf("`bd update %s %s` replaces that item's notes rather than adding to them, "+
			...
			"If the notes really do have to be replaced, include the item's `%s ...` line in the replacement and this will allow it.",
			replaced, notesFlag, goal.AttributionPrefix, appendNotesFlag, goal.AttributionPrefix)
	}
	return ""
}
