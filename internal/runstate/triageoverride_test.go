...
func TestAClearedBudgetStopsRefusing(t *testing.T) {
	...
	for taken := range 2 {
		if _, err := store.RecordMergeRearm(ctx, item, publication, triageDecided(TriageDecisionRearm, decidedRunID), time.Now(), overrideCaps); err != nil {
			t.Fatalf("RecordMergeRearm() %d error = %v", taken+1, err)
		}
	}
	if _, err := store.RecordMergeRearm(ctx, item, publication, triageDecided(TriageDecisionRearm, decidedRunID), time.Now(), overrideCaps); !errors.Is(err, ErrTriageCapReached) {
		t.Fatalf("RecordMergeRearm() past its own cap error = %v, want it still refused", err)
	}
	spent, err := store.RecordMergeRearm(ctx, item, "publication:run-b#8", triageDecided(TriageDecisionRearm, "run-bbbb2222333344445555666677778888"), time.Now(), overrideCaps)
	if err != nil {
