...
// forge dropped. Each of those buys another provider invocation, and none of
// them is bounded by anything the run itself records — a run's repair budget is
// spent inside that run and starts again at zero in the next one. So an item
...
// which budget it is. They are the actions themselves rather than the counters:
// what an operator reads is "no more re-runs", not "the re-run counter".
const (
...
type TriageCounters struct {
	...
	// accepted and then dropped. Neither is a repair: one buys a whole fresh run
	// and the other buys none at all, so bounding them together with the grants
	// would let the cheapest of the three exhaust the budget of the dearest.
...
type TriageCaps struct {
	...
	// ReviewRounds is the ceiling every repair grant is truncated against rather
	// than an action of its own: nothing asks permission to be reviewed, and the
	// rounds an item has spent are a fact rather than a request.
...
// TriageCapError names the budget that refused an action and what was already
// spent against it, because an operator deciding whether to raise a cap needs
// both halves.
...
func (e TriageCapError) Error() string {
	if e.Cap == 0 {
		return fmt.Sprintf("no %s is permitted for %s: the cap is 0", e.Action, e.WorkItemID)
	}
	return fmt.Sprintf("%s has had %d %s(s) of %d permitted", e.WorkItemID, e.Spent, e.Action, e.Cap)
}
...
// two would promise a round nothing would ever let it take, and a grant that
// simply overshot the cap would make the cap decorative. The truncation is
// recorded rather than applied silently — a grant that was cut is what says this
// item is at the end of what it will be given.
//
...
func (s *TriageStore) GrantRepair(ctx context.Context, workItemID string, rounds int, caps TriageCaps) (RepairGrant, error) {
	...
	counters, err := s.update(ctx, workItemID, time.Time{}, func(counters *TriageCounters) error {
		if counters.RepairGrants >= caps.RepairGrants {
			return TriageCapError{Action: TriageRepairGrant, WorkItemID: counters.WorkItemID, Spent: counters.RepairGrants, Cap: caps.RepairGrants}
		}
		remaining := counters.RoundsRemaining(caps.ReviewRounds)
		if remaining == 0 {
			return TriageCapError{Action: "review round", WorkItemID: counters.WorkItemID, Spent: counters.ReviewRounds, Cap: caps.ReviewRounds}
		}
...
// once the item has had the re-runs its cap permits. A re-run buys a whole fresh
// run, repair budget and all, which is why it is bounded on its own rather than
// against the rounds: what it costs is not a round, it is every round the new
// run goes on to spend.
//
...
func (s *TriageStore) RecordRerun(ctx context.Context, workItemID string, caps TriageCaps) (TriageCounters, error) {
	...
	return s.update(ctx, workItemID, time.Time{}, func(counters *TriageCounters) error {
		if counters.Reruns >= caps.Reruns {
			return TriageCapError{Action: TriageRerun, WorkItemID: counters.WorkItemID, Spent: counters.Reruns, Cap: caps.Reruns}
		}
		counters.Reruns++
		return nil
	})
...
// an action that costs nothing to take is the one that can be taken forever, and
// a merge that keeps being dropped is a repository somebody has to look at.
func (s *TriageStore) RecordMergeRearm(ctx context.Context, workItemID string, caps TriageCaps) (TriageCounters, error) {
	...
	return s.update(ctx, workItemID, time.Time{}, func(counters *TriageCounters) error {
		if counters.MergeRearms >= caps.MergeRearms {
			return TriageCapError{Action: TriageMergeRearm, WorkItemID: counters.WorkItemID, Spent: counters.MergeRearms, Cap: caps.MergeRearms}
		}
