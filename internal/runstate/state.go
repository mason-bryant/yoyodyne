...
// commit the harness made of it where the project publishes and so had made
// one. It is what the promotion reads before it moves the target branch.
...
type ChecksPassed struct {
	...
	// and the content above carries the binding on its own. Where there is one
	// it is held to as well, because a commit is what the branch and the forge
	// name the change by.
...
func (p PullRequest) Equal(other PullRequest) bool {
	return p.Remote == other.Remote &&
		...
		p.MergeRearms == other.MergeRearms &&
