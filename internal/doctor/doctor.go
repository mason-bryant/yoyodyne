...
func (d *diagnosis) checkProviders(ctx context.Context, resolved config.Resolved) []Finding {
	...
	for _, named := range backends {
		findings = append(findings, d.checkProvider(ctx, named))
	}
	return findings
}

func (d *diagnosis) checkProvider(ctx context.Context, named domain.Backend) Finding {
