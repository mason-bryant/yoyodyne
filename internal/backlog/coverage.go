...
// the parent field alone. A store that records decomposition only as an edge —
// the tracker's own export is one — reads as a backlog nothing was ever broken
// out of, and a coverage built from that is empty however many epics are
// sitting in the queue beside their children; beads.WorkItem.DecomposedFrom is
// where the cost of getting that wrong is written down.
