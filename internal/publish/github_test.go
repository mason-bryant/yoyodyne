...
func TestGitHubStateReportsAQueuedMergeSeparatelyFromADroppedOne(t *testing.T) {
	...
	if len(listed) != 1 || !contains(listed[0], "number,url,state,mergedAt,autoMergeRequest,headRefOid,mergeCommit,mergeStateStatus,statusCheckRollup") {
		t.Errorf("pr list args = %v, want the queued merge, the head, the merge commit, the merge state and the checks among the requested fields", listed)
	}
}
