...
var Classes = []Class{
	...
	},
	{
		...
		Checks:     []string{"make test"},
		Exercised:  "parsed by a shell in composition's own repository test, so no shell file reaches a reviewer unparsed; executed where it has a suite — the release verb, the notes writer and the release page body, the status tool, and the install script — each run from a Go test rather than from a CI step after integration, and the adoption walkthrough for the one refusal of it that needs no tracker; and read by internal/doclink for the documentation fragments it cites.",
	},
	{
		ID:        "makefile",
