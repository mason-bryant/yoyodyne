...
// operator and carries it back to the agent that wrote the entry, so a role that
// misnames a document is told rather than left believing somebody was asked. It
// is worded for both readers: what a refusal says is what the agent has to answer
// to propose the change again.
...
Writing the block is not the same as the proposal being recorded. The harness refuses one that names a document it has no record of, and one against a document you own yourself, and it can only tell you so after the reply that carried it — your next turn opens with the refusal in its own words, and you propose again from there. So nothing you write may assert that a proposal has been raised: at the moment you write it you do not know, and a document, a comment, or a note claiming a proposal nobody is holding is a false record that outlives this work. Say what you propose in your reply, where it is read beside the block that carried it.`
