...
func TestALongTopicIsCutRatherThanLosingTheMerge(t *testing.T) {
	...
	// The cut is only worth anything if the store then takes the merge: the failure
	// it exists to prevent is a write refused over the length of its label, so the
	// same conclusion is put through Remember and has to land with the cut subject.
	recorded, err := conclusion.merge(context.Background(), newStore(t))
	if err != nil {
		t.Fatalf("merge() error = %v, want the long-topic merge accepted by the store", err)
	}
	if recorded.Subject != revision.Subject {
		t.Errorf("Recorded.Subject = %q, want the cut subject %q written as composed", recorded.Subject, revision.Subject)
	}
}
