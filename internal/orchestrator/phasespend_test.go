...
// pricingBackend serves the developer and the reviewer the way
// orchestratortest.RoleBackend does and, unlike it, records each invocation the
// way the provider adapter does: one terminal event carrying the role the
// invocation was made as and the figure the provider reported for it. That is
// what makes an assertion about the phase split an assertion about the pipeline
// rather than about a log a test wrote.
//
...
func pricingBackend(deaths int, developerCosts []float64, reviewerCost float64, verdicts ...string) *orchestratortest.Backend {
