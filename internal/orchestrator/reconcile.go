...
func (r Reconciler) reportQueuedMerge(state runstate.State, published runstate.PullRequest, observed publish.PullRequest) (Reconciliation, error) {
	...
	if len(held) > 0 {
		...
		result.Detail = fmt.Sprintf("the forge is holding the queued merge of pull request %d into %s: %s failed on it, and the forge merges nothing past a failing required check",
			published.Number, state.Integration.TargetBranch, describeChecks(held))
	}
	return result, nil
}
