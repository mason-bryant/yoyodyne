...
func TestPerformingIntegrateRefusesAChangeTheRecordDoesNotShowPassedItsGate(t *testing.T) {
	...
	earned := &runstate.ChecksPassed{Content: partialContentIdentity, Attempt: 1, Commit: checkedCommit, Commands: []string{"make test"}, At: baseTime}
	for name, test := range map[string]struct {
		...
		"checks that passed over different content": {
			...
			want: "passed over content " + gitworktree.ContentIdentityPrefix + strings.Repeat("f", 64) + " and the change being promoted is " + partialContentIdentity,
		},
