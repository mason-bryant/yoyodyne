...
// epic is attributable to the coverage and to nothing else. Outstanding is
// shadowed because the harness's own answers the carry-out's question, which has
// a different shape from the read model's.
...
func TestTheStandingStatusRefusesTheEpicTheSchedulerWillNotPull(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "a child somebody is already running", childStatus: backlog.StatusClaimed},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if refused.Reason != deferred {
				t.Fatalf("the pass defers %s for %q and the surfaces refuse it for %q; "+
					...
					epic.ID, deferred, refused.Reason)
			}
			...
			if refused.Kind != backlog.HeldCovered {
				t.Fatalf("kind = %q, want the covered pile rather than a stall", refused.Kind)
			}
			// And the epic is not among what the surfaces say the harness pulls next,
			// which is the other half of the same disagreement.
			for _, startable := range standing.StartableItems {
				if startable.WorkItemID == epic.ID {
					t.Fatalf("startable = %+v, want the covered epic absent from what the harness pulls next", standing.StartableItems)
				}
			}
		})
	}
