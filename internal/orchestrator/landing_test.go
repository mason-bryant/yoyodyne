...
func TestAnApprovalThatSaysNothingIsAskedForAgain(t *testing.T) {
	...
	provider := orchestratortest.RoleBackend(writeFeature, unstated, approveVerdict)
...
func TestAMarkerTheHarnessCannotUseTakesTheParkingInstead(t *testing.T) {
	...
	for _, unusable := range []struct {
		name    string
		marker  func(tracker *orchestratortest.Tracker) string
		saysWhy string
	}{
		{
			...
			marker:  func(tracker *orchestratortest.Tracker) string { return tracker.Item.ID },
		...
		},
		{
			...
			marker:  func(*orchestratortest.Tracker) string { return "yoyodyne-never-admitted" },
...
func TestDecidingWhereAnUndischargedItemGoesReportsATrackerItCouldNotRead(t *testing.T) {
	...
	// cannot answer for.
	tracker := &orchestratortest.Tracker{Item: beads.WorkItem{ID: "yoyodyne-other", Title: "Another", Status: "open"}}
	state := runstate.State{
		RunID:            "run-abcdef0123456789abcdef0123456789",
...
func TestTheParkingDefaultRecordsTheReasonItSuperseded(t *testing.T) {
	...
	unparked := &orchestratortest.Tracker{Item: beads.WorkItem{ID: "yoyodyne-task", Title: "Task", Status: "in_progress"}}
...
func TestALaterAttemptReplacesTheLandingTheEarlierOneClaimed(t *testing.T) {
	...
	tracker := newOutcomeTracker()
	provider := orchestratortest.RoleBackend(writeFeature, repairVerdict, approveVerdict)
