...
// category.
func TestDescribeProviderFailureKeepsTheProvidersOwnWordsBesideItsCategory(t *testing.T) {
	t.Parallel()

	overloaded := "API Error: 529 Overloaded. This is a server-side issue, usually temporary."
	described := describeProviderFailure(backend.RunResult{StopReason: "api_error", FinalText: overloaded})
	...
	for _, testCase := range []struct {
		...
		{name: "category only", result: backend.RunResult{StopReason: "api_error"}, want: "api_error"},
		{name: "same words", result: backend.RunResult{StopReason: "api_error", FinalText: "api_error"}, want: "api_error"},
		{
			name:   "message repeats the category",
			result: backend.RunResult{StopReason: "api_error", FinalText: "the provider reported api_error"},
			want:   "the provider reported api_error",
		},
		{name: "message only", result: backend.RunResult{FinalText: "it went wrong"}, want: "it went wrong"},
		{name: "nothing", result: backend.RunResult{}, want: "unknown provider failure"},
	} {
		if described := describeProviderFailure(testCase.result); described != testCase.want {
	...
	long := describeProviderFailure(backend.RunResult{
		StopReason: "api_error",
		FinalText:  strings.Repeat("a", maxProviderFailureDetailBytes*2),
	})
	if len(long) > maxProviderFailureDetailBytes+64 || !strings.HasSuffix(long, "...") {
