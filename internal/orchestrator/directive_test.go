...
// directive is most likely to arrive: the developer is being handed findings and
// asked again, which can take as long as the original attempt. A directive
// recorded in that window has to stop the run before it buys another round and
// before it promotes anything, or it would be enforced against every part of the
// run except the longest one.
func TestADirectiveRecordedDuringARepairAttemptStopsTheRunBeforeItPromotes(t *testing.T) {
	t.Parallel()
	...
	// the next change it was shown, so a run that failed to ask again here would
	...
	if pausedState.Status.Terminal() || pausedState.DirectivePause == nil {
