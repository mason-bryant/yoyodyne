...
pulling:
	for {
		...
		for _, entry := range queue.Entries {
			...
			running++
			started++
			s.host(ctx, pull, entry.ID, index, selection, hosted, completions)
		}
		if started > 0 {
	...
	case schedule.Stopped == ScheduleRedeployed && schedule.Drain != nil && schedule.Drain.BoundReached:
		...
		if len(schedule.Drain.Stopped) == 0 {
			return fmt.Sprintf("%s; the drain bound of %s ran out with only a run at its promotion still going, which was waited out rather than stopped",
				schedule.Stopped, schedule.Drain.Bound)
		}
		return fmt.Sprintf("%s; the drain bound of %s ran out with %d run(s) still going, which were stopped and preserved for the session that comes back: %s",
			schedule.Stopped, schedule.Drain.Bound, len(schedule.Drain.Stopped), strings.Join(schedule.Drain.Stopped, ", "))
	}
...
func (a account) same(other account) bool {
	if a.reason != other.reason || a.running != other.running ||
		...
		a.window != other.window || a.drain != other.drain {
		return false
	}
...
func (s Scheduler) host(ctx context.Context, pull Pull, workItemID string, index int, selection runstate.Selection, hosted map[int]context.CancelCauseFunc, completions chan<- completed) {
...
func (s Scheduler) stopHosted(schedule *Schedule, drain *redeployDrain, hosted map[int]context.CancelCauseFunc, mine map[string]int, runs ScheduleRuns) int {
	drain.reached()
	schedule.Drain.BoundReached = true
	if runs == nil {
		return 0
	}
	_, inFlight, err := occupiedItems(runs)
	if err != nil {
...
func occupiedItems(runs ScheduleRuns) (map[string]string, map[string]runstate.State, error) {
	incomplete, err := runs.Incomplete()
	if err != nil {
		return nil, nil, fmt.Errorf("read what is already in flight: %w", err)
	}
	occupied := make(map[string]string, len(incomplete))
	states := make(map[string]runstate.State, len(incomplete))
	for _, state := range incomplete {
		if !state.Status.InFlight() {
			continue
		}
		occupied[state.WorkItemID] = state.RunID
		states[state.WorkItemID] = state
	}
	return occupied, states, nil
}
