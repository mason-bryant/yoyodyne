...
// Listing and holding are two moments, and a live carrier can finish in between:
// it answers its round, releases the lease, and the sweep then finds that lease
// free while its own listing still shows the round unanswered. Deciding from the
// listing would reclaim a round that had just come back — the answer and the cost
// the provider was paid for overwritten with "the process carrying it is gone",
// which is the exact double-write the lease is taken to prevent.
//
// The interleaving is real rather than described: the carrier holds a real lease,
// the completion happens between the pass's own List and its Hold, and the
// listing the pass reasons from is genuinely the pre-completion one.
func TestARoundThatCompletesDuringDiscoveryIsNotReclaimedOverTheAnswer(t *testing.T) {
	t.Parallel()

	store, reports := supervisionStores(t)
	racing := openExchange("exchange-"+strings.Repeat("3", 32), 3)
	racing.Rounds = []exchange.Round{askedRound(1, "the live carrier")}
	save(t, store, racing)

	// The live carrier, mid-round: it holds the lease, exactly as the process
	// taking a round does.
	carrier, taken, err := store.Hold(racing.ID)
	if err != nil || !taken {
		t.Fatalf("Hold() = %t, %v; want the carrier holding its own lease", taken, err)
	}

	answered := racing
	answered.Rounds = []exchange.Round{answeredRound(1, "what you are missing is the lease")}
	answered.Rounds[0].CostUSD = 1.25

	loop := supervisionLoop(store, reports)
	loop.Store = &carrierFinishesAfterListing{inner: store, finish: func() {
		save(t, store, answered)
	...
	if recorded(pass, racing.ID, SupervisionReclaimed) {
		t.Fatalf("results = %#v, want a round that came back left alone", pass.Results)
	}

	after := load(t, store, racing.ID)
	if after.Rounds[0].Answer != "what you are missing is the lease" {
		t.Errorf("Answer = %q, want the answer that came back still there", after.Rounds[0].Answer)
	}
	if after.Rounds[0].Problem != "" {
		t.Errorf("Problem = %q, want an answered round carrying no reclaim reason", after.Rounds[0].Problem)
	}
	if after.Rounds[0].CostUSD != 1.25 {
		t.Errorf("CostUSD = %v, want what the provider was paid still recorded", after.Rounds[0].CostUSD)
	}
}

// carrierFinishesAfterListing lets a live carrier finish its round in the window
// between the pass's listing and the leases it takes afterwards. Everything else
// is the real store, so what the pass reads back is what is really on disk.
type carrierFinishesAfterListing struct {
	inner    *runstate.ExchangeStore
	finish   func()
	finished bool
}

func (c *carrierFinishesAfterListing) List() ([]exchange.Exchange, error) {
	listed, err := c.inner.List()
	// After the listing is taken, so the copy the pass carries forward is the one
	// showing a round nobody had answered yet.
	if !c.finished {
		c.finished = true
		c.finish()
	}
...
func (c *carrierFinishesAfterListing) Load(id string) (exchange.Exchange, error) {
	return c.inner.Load(id)
}

func (c *carrierFinishesAfterListing) Hold(id string) (exchange.Release, bool, error) {
