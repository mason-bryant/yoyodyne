...
//
// The age is the run's last write, which is the closest thing the record keeps to
// when the park began: nothing durable says when a run entered its wait. That is
// a real limit rather than a choice — anything that rewrites a parked run's state
// restarts this budget — and closing it means recording the moment the pause
// started, which is runstate's to hold rather than this file's to infer.
