...
func TestCursorsRecordBothWhatWasReadAndWhatWasSaid(t *testing.T) {
	...
	run := reloaded.Streams["run:run-a"]
	if !run.Has("run.started:x") || !run.Has("checks.passed:y") {
		t.Fatalf("run cursor = %#v, want every crossing already said", run)
	}
	if run.Has("promotion.made:z") {
		t.Fatal("a crossing that was never posted must not read as posted")
	}
	// The reading everything already said was said about is what the next pass
	// is a comparison against, so it has to come back exactly as it was left.
	if run.Reported == nil || run.Reported.Phase != runstate.PhaseReviewing {
		t.Fatalf("run cursor = %#v, want the reported reading of the run kept", run)
	}
	// A pair that has been said in full is forgotten, so a cursor does not grow
	// a line for every hold the operator ever placed.
	if run.Without("run.started:x").Has("run.started:x") {
		t.Fatal("Without() must forget the mark it was given")
	}
	if mark, found := run.Marked("checks."); !found || mark != "checks.passed:y" {
		t.Fatalf("Marked() = %q, %t, want the mark found under its prefix", mark, found)
	}
