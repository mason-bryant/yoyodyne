...
// Lookup reports the thread a topic already opened in this channel.
func (m ThreadMap) Lookup(channel string, topic notify.Topic) (Thread, bool) {
	thread, found := m.Threads[string(topic)]
...
func (m *ThreadMap) Record(topic notify.Topic, thread Thread) {
	if m.Threads == nil {
		m.Threads = map[string]Thread{}
	}
	m.Threads[string(topic)] = thread
}

// Cursor is how far the sink has read one durable stream. A stream that is a
// log advances by position; one that reports a fixed set of milestones about a
// single subject records which of them have been said, because those do not
// arrive in a fixed order and a run can park and continue more than once.
type Cursor struct {
...
func (c Cursor) With(mark string) Cursor {
	...
	advanced := Cursor{Position: c.Position, Delivered: append(append([]string(nil), c.Delivered...), mark)}
