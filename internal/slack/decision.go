...
func (s *steering) decideThread(ctx context.Context, message inboundMessage) {
	...
	if message.threadTS == "" {
		...
		s.answerDecision(ctx, message, misplaced(latest, s.standingMark()))
		return
	}
...
// standingMark is the state the heartbeat is currently standing on, or nothing
// where it is standing on none or cannot say.
//
...
// A cursor that cannot be read answers as nothing standing, which is the reading
// that never sends somebody to a thread on a state it cannot vouch for.
func (s *steering) standingMark() string {
	...
	return cursors.Streams[heartbeatStream].Standing
}
...
func misplaced(latest Decision, standing string) string {
