...
(function () {
  ...
  // movers is the read model's own vocabulary for whose move each thing
  // waiting on a person is, in the order the model says them: the operator
  // first, because the page is read by the operator and what is his is what
  // the figure has to say, then the three roles and the harness. Each entry
  // arrives carrying its mover; nothing here reads it off the words beside it.
  // On 2026-09-20 the tile said sixty-four things needed a human, and three of
  // them were the human's.
  var movers = [
    ...
    { mover: "unnamed", label: "a role the item does not name" }
  ];
  ...
  // not zero. A mover the vocabulary does not name is counted under its own
  // word rather than dropped, so the counts always add up to the line.
  function byMover(entries) {
    var counted = {};
    entries.forEach(function (entry) {
      var mover = entry.mover || "unnamed";
      counted[mover] = (counted[mover] || 0) + 1;
    });
    var counts = [];
    movers.forEach(function (named) {
      var number = counted[named.mover] || 0;
      delete counted[named.mover];
      if (number > 0 || named.mover === "operator") {
        counts.push({ mover: named.mover, label: named.label, number: number });
      }
    });
    Object.keys(counted).sort().forEach(function (mover) {
      counts.push({ mover: mover, label: "the " + mover.replace(/-/g, " ") + "'s", number: counted[mover] });
    });
    return counts;
  }
