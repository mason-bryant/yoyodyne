...
(function () {
  ...
  // stage is one stage of the pipeline. Its label and figure are one button
  // that opens the list of the items the figure counts — or, for a stage whose
  // source could not be read or is still being priced, the pop-up saying so
  // with the reason in full.
  function stage(label, figure, unit, className, grouping) {
    var item = el("li", "stage" + (className ? " " + className : ""));
    var head = groupingOpener(grouping, "stage-open");
    head.appendChild(el("span", "stage-label", label));
    head.appendChild(el("span", "stage-figure", figure));
  ...
  function pile(grouping, figure, label, className) {
    ...
    var opener = groupingOpener(grouping, "pile-open");
    opener.appendChild(el("span", "pile-label", label));
    entry.appendChild(opener);
    return entry;
  }
  ...
  function renderPipeline() {
    ...
    if (throughput && !throughput.runs_problem) {
      ...
      var weekOpener = groupingOpener("landed:week", "stage-detail-open");
      weekOpener.appendChild(el("span", "stage-detail", count(week.landed, "run") + " in the last 7 days"));
      landed.appendChild(weekOpener);
  ...
  function groupingOpener(key, className) {
    var button = el("button", "grouping-open" + (className ? " " + className : ""));
    button.setAttribute("type", "button");
