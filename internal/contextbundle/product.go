...
func renderTriageDocket(request ProductRequest) (string, *triage.WindowPosition) {
	...
	fits := func(standing triage.Stoppage) bool {
		...
		if standing.Folded > 0 {
			section += fmt.Sprintf("      %d further docket entry(s) about this same run are folded into this one; it has waited since %s.\n",
				standing.Folded, standing.Since.UTC().Format(time.RFC3339))
		}
