...
func TestStatusReadsTheSameStoreRunsAreRecordedIn(t *testing.T) {
	...
	// The triage counters are the same store's, and reached the same way a run
	// records them, so the verb that reports what an item has been given cannot
	// be reading a different directory from the one that writes it.
	if store.Triage().Root() != parts.store.Triage().Root() {
		t.Fatalf("status reads triage counters at %s, but runs record them at %s",
			store.Triage().Root(), parts.store.Triage().Root())
	}
	...
	// out of budget.
	if want := orchestrator.TriageCaps(mustLoadConfig(t, configPath).Execution); caps != want {
...
func TestStatusReportsWhatTriageHasSpentOnANamedItem(t *testing.T) {
	...
	// An item nobody has triaged says so, rather than printing three zeroes that
	// read as a budget somebody has been spending.
	...
	caps := runstate.TriageCaps{RepairGrants: 1, Reruns: 1, MergeRearms: 2, ReviewRounds: 4}
	triage := store.Triage()
	...
	granted, err := triage.GrantRepair(context.Background(), "yoyodyne-ifd.2.7", 2, caps)
	if err != nil {
		t.Fatalf("GrantRepair() error = %v", err)
	}
	if _, err := triage.RecordRerun(context.Background(), "yoyodyne-ifd.2.7", caps); err != nil {
	...
	// The grant was cut to the one round the cap had room for, which is what the
	// listing above is reporting.
	...
	if *reported.TriageCaps != (runstate.TriageCaps{RepairGrants: 1, Reruns: 1, MergeRearms: 2, ReviewRounds: 4}) {
		t.Fatalf("reported caps = %+v, want the configured defaults", *reported.TriageCaps)
	}
}
