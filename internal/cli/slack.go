...
func buildSlackSink(configPath string, poll, heartbeat time.Duration, version string, stdout io.Writer) (*slack.Sink, string, error) {
	...
	sink, err := slack.New(slack.Options{
		...
		Feed: &slack.HarnessFeed{
			...
			//
			// The directives are read a second time for the state nothing else
			// brings to the person who has to end it: a directive nobody has settled
			// stops the work it names, and settling one is the operator's and nobody
			// else's.
...
It needs the app's im:write scope; without it the channel reports exactly as it
does now and this one tier stays quiet, saying so once.
