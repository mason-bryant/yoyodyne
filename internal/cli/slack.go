...
func runSlack(ctx context.Context, args []string, stdout, stderr io.Writer, version string) int {
	...
	sink, channel, err := buildSlackSink(*configPath, *poll, version, stdout)
	if err != nil {
...
// happens to sit.
func buildSlackSink(configPath string, poll time.Duration, version string, stdout io.Writer) (*slack.Sink, string, error) {
