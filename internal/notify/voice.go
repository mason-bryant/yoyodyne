...
var harnessVoice = voice{
	...
	lines: map[Kind]string{
		...
		KindMergeHeld:                "The forge is holding the queued merge of {pr}: {cause}. The change is promoted; the fix lands on the target branch, and the forge performs the merge once the check passes there.",
...
var nextMoves = map[Kind]string{
	...
	// perform it by itself the moment the check passes, and what makes it pass
	// is a fix landed on the target branch, where no run's repair of its own
	// change reaches.
	KindMergeHeld:    "the operator's — the fix lands on the target branch, and the forge performs every held merge once the check passes there.",
