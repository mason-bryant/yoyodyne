...
// Once per refusal: the same refusal read twice on one round is one refusal; the
// same paths refused again on the next round are a second round spent for the
// same stated cause; a refusal that cleared says nothing of its own, because
// what follows it is the checks, and they say what they find.
func TestARefusedPathIsSaidOncePerRefusal(t *testing.T) {
	...
	// A different path on the same round is a different refusal.
	differently := first
	differently.PathRefusal = &runstate.PathRefusal{Paths: []string{"docs/product/brief.md"}}
	if kinds, _ := crossed(t, first, differently); len(kinds) != 1 || kinds[0] != KindPathRefused {
		t.Fatalf("a different refusal on the same round crossed %v", kinds)
	}
