...
// turn being taken. Each proposal is delivered once per conversation: it stays
// pending until somebody decides it, and repeating the same list every turn
// would spend the context that the rest of the conversation needs on something
// already said.
func (s *Session) renderProposedAmendments() string {
	...
	for _, proposal := range delivered {
		...
		// repeating the list on the next one would spend it twice.
		s.deliveredAmendments[proposal.ID] = true
	}
