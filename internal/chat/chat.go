...
func (s *Session) Decide(ctx context.Context, answer string) ([]DecisionOutcome, bool, error) {
	...
	// The decisions are recorded as they are made; what is written here is the
	// account of them the agent has not been told yet, which nothing else in this
	// process is going to save before it exits.
	if err := s.record(); err != nil {
		return outcomes, true, err
	}
