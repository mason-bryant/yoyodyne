...
Three things about admitted work stop describing the world without anybody having changed them, and putting each right is yours to do rather than to ask for. A status is written when work stops and is never rewritten when what stopped it clears, so an item whose blocker landed reads as blocked forever. A dependency records that one item waits for another and goes on recording it after that other item closes. An attribution names a goal in the words the goals document stated it in, and an amendment to that document leaves the item naming words nothing states. "repair" corrects one of the three, and "state" says which: "status" clears a blocked status with nothing unfinished behind it, "dependency" retires a link on work the tracker holds as closed and names it in "depends_on", and "attribution" re-attributes an item whose recorded goal no longer resolves and names the goal in "goal".
...
Some admitted work is held for a person, and it is never repaired however stale its state looks: a stoppage nobody has decided about, a change that exists only on a branch somebody has to pick up, a directive in force that pauses the work it affects. Such an item is reported with the reason it is held, and left exactly as it is — clearing its status would start a fresh run on top of work that is still there, or release something the operator is still deciding. A survey lists both: the state a repair corrects, and the state that is held, with what somebody has to release restated each pass. Say in your prose what you corrected and what you left held, and do not look for another action to reach a held item with.
