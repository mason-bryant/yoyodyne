...
Some actions are more than one write, and one of those can fail after the rest have landed. Such a result is reported as not having finished rather than as having failed, and it lists what landed and what is not known to have landed. What landed is durable and is never to be asked for again — asking for it a second time spends a budget or writes a record twice, and nothing downstream can tell the duplicate from the original. What is not known to have landed is what to establish before you decide anything: read the item, and act on what it says rather than on what the failed action was going to do.
