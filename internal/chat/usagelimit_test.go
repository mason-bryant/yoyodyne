...
// the refusal reaches somebody who is not at this terminal. A conversation given
// no bounds to wait under — which is every turn the harness takes for itself —
// fails the turn exactly as it did before, and nothing here waits.
...
func TestARefusedTurnWaitsOutTheLimitAndIsAskedAgain(t *testing.T) {
	...
	// while it waited.
	refusedRequest, reissued := provider.requests[0], provider.requests[1]
	if reissued.Prompt != refusedRequest.Prompt || reissued.SessionID != refusedRequest.SessionID {
		t.Fatalf("reissued request = %+v, want the prompt and session that were refused", reissued)
	}
	// And its events are numbered after the refused attempt's rather than over
	// them, so the one log the two attempts wrote still says what order things
	// happened in.
	if reissued.LastSequence <= refusedRequest.LastSequence {
		t.Fatalf("reissued LastSequence = %d, want it past the refused attempt's %d", reissued.LastSequence, refusedRequest.LastSequence)
	}
...
func TestTrackerActionsAppliedBeforeAWaitAreNotRepeatedAfterIt(t *testing.T) {
	...
	// What was asked again is the continuation, on the session the round that
	// applied the actions left behind.
	if provider.requests[2].Prompt != provider.requests[1].Prompt || provider.requests[2].SessionID != "session-1" {
		t.Fatalf("reissued request = %+v, want the refused continuation on the same session", provider.requests[2])
	}
...
func TestALimitTheHarnessWillNotWaitOutFailsWithTheResetStated(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// A caller that is not a person still reads it as the role never having
			// been asked, which is what decides whether the turn is worth asking again.
			if !errors.Is(err, ErrProviderCapacity) {
				t.Fatalf("Send() error = %v, want it to still say the provider declined the turn", err)
			}
			if clock.waited() != 0 {
...
// The budget covers the message rather than one wait, so a provider that keeps
// refusing reaches the configured maximum instead of walking one message past it
// a probe at a time: three half-hour probes fit inside ninety minutes and the
// fourth does not, and the refusal says what has already been waited.
func TestAWaitSpendsOneBudgetAcrossEveryProbeAMessageTakes(t *testing.T) {
	t.Parallel()

	clock := &waitingClock{now: fixedClock{}.Now()}
	resetsAt := clock.now.Add(6 * time.Hour)
	provider := &fakeBackend{results: []backendapi.RunResult{
		refusedForCapacity(resetsAt),
		refusedForCapacity(resetsAt),
		refusedForCapacity(resetsAt),
		refusedForCapacity(resetsAt),
		{SessionID: "session-1", FinalText: "Never reached."},
	}}
	options := waitingOptions(testOptions(t, provider), clock)
	options.UsageLimitPause.Maximum = 90 * time.Minute
	options.UsageLimitPause.InProcess = 90 * time.Minute

	_, err := openTestSession(t, options).Send(context.Background(), "what is next?")
	var refused *UsageLimitError
	if !errors.As(err, &refused) {
		t.Fatalf("Send() error = %v, want the fourth probe refused as past the budget", err)
	}
	if !strings.Contains(refused.Error(), "already waited 1h30m0s") {
		t.Fatalf("refusal = %q, want what the message had already waited stated", refused.Error())
	}
	if clock.waited() != 90*time.Minute {
		t.Fatalf("waited %s, want exactly the three probes the budget had room for", clock.waited())
	}
	if len(provider.requests) != 4 {
		t.Fatalf("invocations = %d, want the first attempt and three probes", len(provider.requests))
	}
}

// A turn that is waiting says so on the display, and says when it will actually
// ask again. Without it a turn waiting out hours of provider silence looks
// exactly like a hung command, which is the thing the activity display exists to
// tell apart. The moment named is the end of the probe rather than the quoted
// reset: the reset is 45 minutes out and the probe is thirty, so a line naming
// the reset would promise a turn that asks again fifteen minutes later than it
// does.
func TestAWaitingTurnSaysSoOnTheDisplay(t *testing.T) {
	...
	session := openTestSession(t, waitingOptions(testOptions(t, provider), clock))
	asksAgainAt := clock.now.Add(30 * time.Minute).Local().Format(time.Kitchen)
	resetsAt := clock.now.Add(45 * time.Minute).Local().Format(time.Kitchen)
	...
	for _, required := range []string{
		"… waiting out an exhausted five_hour usage limit; asking again at " + asksAgainAt + "\n",
		"Two goals, then.",
	} {
	...
	if strings.Contains(transcript, "asking again at "+resetsAt) {
		t.Fatalf("transcript = %q, want the display to name when the turn asks again rather than the quoted reset", transcript)
	}
}

// A wait the harness will not take leaves an interactive conversation open: the
// operator is told what refused the turn and when the provider said it lifts,
// and the prompt comes back, because nothing about the conversation is broken.
func TestAnUnwaitableLimitLeavesTheConversationOpen(t *testing.T) {
	t.Parallel()

	clock := &waitingClock{now: fixedClock{}.Now()}
	provider := &fakeBackend{results: []backendapi.RunResult{
		refusedForCapacity(clock.now.Add(-time.Minute)),
		{SessionID: "session-1", FinalText: "Two goals, then."},
	}}
	session := openTestSession(t, waitingOptions(testOptions(t, provider), clock))
	...
	var out strings.Builder
	if err := session.Converse(context.Background(), testConsole(strings.NewReader("what is next?\nand now?\n/exit\n"), &out)); err != nil {
		t.Fatalf("Converse() error = %v, want an unwaitable limit reported rather than ending the conversation", err)
	}
	transcript := out.String()
	for _, required := range []string{
		"this turn was refused by an exhausted five_hour usage limit, which it reports resetting at",
		"saying this again takes the turn that was refused",
		"Two goals, then.",
	} {
		if !strings.Contains(transcript, required) {
			t.Fatalf("transcript = %q, want it to contain %q", transcript, required)
...
// An operator who gives up on a wait they were shown ends the turn there. The
// cancellation is reported as the interruption it is rather than the turn
// quietly asking again on a context nobody is listening to.
func TestAWaitTheOperatorInterruptsEndsTheTurn(t *testing.T) {
	t.Parallel()
	...
	_, err := openTestSession(t, options).Send(ctx, "what is next?")
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("Send() error = %v, want the operator's cancellation reported", err)
	}
	if len(provider.requests) != 1 {
		t.Fatalf("invocations = %d, want nothing asked after the operator gave up", len(provider.requests))
	}
}
...
func waitingOptions(options Options, clock *waitingClock) Options {
	...
	options.UsageLimitUnknownResetPause = 30 * time.Minute
	return options
}
