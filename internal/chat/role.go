...
// exemptionClause states the classes of work this project has carved out of
// whatever its gate is, and is empty for a project that has carved out none —
// which is every project until its operator says otherwise. It is stated only
// where it is in force, deliberately: telling a role about a class the harness
// will ignore is an invitation to claim one, and a claim that changes nothing is
