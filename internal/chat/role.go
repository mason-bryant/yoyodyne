...
{"actions":[
  {"action":"read","id":"beads-id"},
  {"action":"survey"},
  {"action":"create","title":"one line","description":"what the work is and what done means","goal":"the goal this work serves","parent":"beads-id","priority":2,"executor":"conversation:architect","labels":["reliability"],"reason":"why you are doing this"},
  ...
  {"action":"triage","id":"beads-id","run":"the run the docket entry names","decision":"repair|rerun|rescope|rearm|wait|escalate|cross","budget":"cross only: the cap being crossed","reason":"why"},
...
Work you carve out of a run that failed or was deferred is written against that run's change, and that change is on the branch the run preserved rather than on the branch a fresh worktree is cut from. A child that assumes files existing only there is never ready, however clean it reads: the run that pulls it starts in a worktree that does not have them. So it waits for its parent's change to land, by whatever vehicle lands it — the preserved branch cherry-picked, the pull request revived, or the substrate rebuilt from nothing. You do not have to add that link and you cannot leave it out: where the harness's own run records say the parent's change never reached the integration target, a creation under it is linked to wait for it and the child is told why, and the result of the creation says so. Say in the item's description which files it assumes and where they are, so whoever lands the parent knows what is waiting on them, and say to the operator which vehicle you think is cheapest — that decision is theirs and the product manager's, not yours.
