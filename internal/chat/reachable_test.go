...
func TestARealHoldEndedAtThePromptLeavesTheConversationFree(t *testing.T) {
	...
	store := options.Store.(*runstate.ConversationStore)
	identity := options.identity()
	hold, err := store.Claim(context.Background(), runstate.ConversationIdentity{Agent: identity.Agent, Role: identity.Role})
	if err != nil {
...
// Propose-then-approve across a conversation put-down. A proposal is the part
// of a conversation that is most obviously a person's to decide and least
// obviously this process's to hold: it is made in whichever process had the
// turn, and approved in whichever one the operator is sitting at. So a console
// that put its conversation down while somebody else proposed must come back
// with that proposal rather than without it — and, worse than merely not seeing
// it, a console carrying on from its own stale copy would write that copy back
// at its next put-down and destroy the proposal outright, leaving the operator's
// approval nothing to name.
func TestAProposalMadeWhileTheConversationWasDownIsStillThereToApprove(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	tracker := &fakeTracker{}
	options := perItemApprovalOptions(t, root, tracker, "")
	options.Backend = &fakeBackend{results: []backendapi.RunResult{
		{SessionID: "session-1", ResolvedModel: "claude-opus-5-20260514", FinalText: "Nothing is missing."},
		{SessionID: "session-1", ResolvedModel: "claude-opus-5-20260514", FinalText: "Still nothing."},
	}}
	hold := &promptHold{}
	options.Hold = hold
	// The other process — a `--message` invocation, the harness relaying, or the
	// operator's assistant — proposes work while the console waits for its second
	// message. It goes to the prompt before the console's first turn as well,
	// which is why the release is named: until that turn there is no session for
	...
	hold.meanwhile = func(release int) {
		...
		other := openTestSession(t, proposing)
		reply, err := other.Send(context.Background(), "what should we do about usage limits?")
		if err != nil {
			t.Errorf("the other process could not propose: %v", err)
			return
		}
		if len(reply.Proposals) != 1 {
			t.Errorf("the other process proposed %d item(s), want 1", len(reply.Proposals))
		}
	}
	session := openTestSession(t, options)
	...
	// The proposal came back with the conversation, and is still undecided:
	// nothing was created while the operator had not answered.
	pending := session.Proposals()
	if len(pending) != 1 {
		t.Fatalf("%d proposal(s) survived the put-down, want the undecided one", len(pending))
	}
	if pending[0].Proposal.Title != "Pause on a usage limit" {
		t.Fatalf("the proposal came back as %#v, want what the other process made", pending[0].Proposal)
	}
	if len(tracker.created) != 0 {
		t.Fatalf("%d item(s) were created before anybody approved anything", len(tracker.created))
	}
	// And it is still on the record after the console's own last put-down wrote
	// the conversation back, which is where a stale copy would have erased it.
	recorded, err := options.Store.(*runstate.ConversationStore).Load(options.identity())
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}
	if len(recorded.PendingProposals) != 1 {
		t.Fatalf("%d proposal(s) are on the record, want the undecided one", len(recorded.PendingProposals))
	}
