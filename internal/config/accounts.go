...
// provider home — what `claude` would use if nothing said otherwise — which is
// what the default alias resolves to.
...
// when nothing chooses between several. V1 configured one account and assigned
// every role to it, so a run had exactly one alias to record: the configured
// one, or the default where nothing named any. A pooled configuration names
// none here, because guessing which of them a run used is exactly the claim this
// record exists to make honestly — ChooseAccount is what picks, and what it
// picked is what the run records.
