...
Which agents can reach it, and what a developer run uses instead, is
[below](#the-tracker-is-not-a-developer-run-tool) — read that first if this run
is a developer run.
...
manager's conversation, and an interactive session. In a developer run every one
of them fails; see [the next section](#the-tracker-is-not-a-developer-run-tool).
...
The sections between the `BEADS INTEGRATION` and `BEADS CODEX SETUP` markers, and
the beads skill at `.agents/skills/beads/SKILL.md`, are generated by `bd setup`
and rewritten wholesale when it runs again, so nothing here is edited into them.
Where they say to use `bd` for all task tracking, read that as describing the
harness and conversation contexts that can reach the store. In a developer run
