...
A later line that is not an attribution.\"" \
  ...
  step "the attribution reader can be asked what it reads"
  # The mode notesguard_repository_test.go drives to hold named_in() to
  # goal.NamedIn. It is exercised here too so that a break in it is a failed
  # claim in the guard's own suite rather than only a Go test failing for a
  # reason that reads like a Go problem.
  read_back="$(printf 'Admitted.\nGoal served: %s\n' "$goal" | python3 "$repository/scripts/bd-notes-guard.py" --named-in)"
  if [ "$read_back" = "$goal" ]; then
    pass "--named-in answers with the attribution in the notes it is given"
  else
    fail "--named-in answers with the attribution in the notes it is given -- got: $read_back"
  fi
  read_back="$(printf 'Filed directly, naming no goal.\n' | python3 "$repository/scripts/bd-notes-guard.py" --named-in)"
  if [ -z "$read_back" ]; then
    pass "--named-in answers with nothing where the notes name no goal"
  else
    fail "--named-in answers with nothing where the notes name no goal -- got: $read_back"
  fi
fi
