...
Run with `--named-in` it answers a different, smaller question -- which
attribution it reads in the notes on stdin -- and writes that on stdout instead
of deciding anything. That mode exists for notesguard_repository_test.go, which
puts the same notes through goal.NamedIn and through this file and requires the
two to agree. See named_in() for why that is worth a check rather than a comment.
"""
...
def named_in(notes):
    ...
    Every part of that rule is duplicated across two languages -- which line
    counts, what is trimmed off it, which of several wins -- and every part of
    it drifts in the same direction when it drifts: this reads no attribution,
    the write looks like one against an unattributed item, and the guard allows
    the destruction while still passing its own suite. So the agreement is
    checked rather than asserted. `--named-in` is how: the Go side puts a table
    of notes through both and requires the same answer.
    """
    named = ""
...
def main(argv):
    # The one argument this takes, and it is not for the hook: it reads notes on
    # stdin and writes back the attribution found in them, so the Go side can
    # hold named_in() to goal.NamedIn over a shared table of notes rather than
    # to the prefix constant alone. A hook payload never carries an argument, so
    # the two modes cannot be confused for one another.
    if argv[1:] == ["--named-in"]:
        sys.stdout.write(named_in(sys.stdin.read()) + "\n")
        return 0
    if len(argv) > 1:
        sys.stderr.write("bd-notes-guard: unknown argument %r\n" % (argv[1],))
        return 64
    return decide()
